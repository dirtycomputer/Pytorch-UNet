...
def train_net(net,                          #模型
              ...
              val_percent: float = 0.1,         #验证集所占数据集比例 #TODO 0.1
    ...
    #project添加项目名称
    experiment = wandb.init(project='Pytorch-Unet', resume='allow', anonymous='must')
    #添加wandb参数
        ...
        with tqdm(total=n_train, ascii=True, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            ...
            for batch in train_loader:
                ...
                with torch.cuda.amp.autocast(enabled=amp):
                    #TODO
                    #mask_pred形状为(1, 2, H, W)
                    masks_pred = net(images)
                    #loss为交叉熵+dice
                    loss = criterion(masks_pred, true_masks) \
                           + dice_loss(F.softmax(masks_pred, dim=1).float(),F.one_hot(true_masks, net.module.n_classes).permute(0, 3, 1, 2).float(),
                                       multiclass=True)
...
if __name__ == '__main__':
    ...
    if torch.cuda.device_count() > 1:
        ...
        net = nn.DataParallel(net, device_ids=[0,1,2]).to(device)
    net.to(device)
